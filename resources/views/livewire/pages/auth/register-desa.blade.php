<div x-data="{ verificationMethod: @entangle('verificationMethod') }">
    <div class="mb-6">
    ...
    {{-- MODIFIKASI: Menambahkan x-data kosong agar semua x-ref terdaftar dalam satu scope --}}
    <form wire:submit="register" class="space-y-4" x-data>
        @csrf
        ...
        <fieldset class="p-4 border rounded-lg bg-gray-50 space-y-4" @if($otpVerified) disabled @endif>
            ...
            <div>
                ...
                <div class="flex items-center">
                    {{-- Field ini dilewati karena terisi otomatis --}}
        ...
        <div class="p-4 border rounded-lg bg-gray-50 mt-6">
            ...
            <div class="mt-4">
                ...
                {{-- Event keydown sudah ada, ini sudah benar --}}
            ...
            <div class="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <x-text-input wire:model="otp" id="otp" class="block mt-1 w-full" type="text" required
                        x-ref="otp"
                        @keydown.enter.prevent="$wire.verifyOtp()" />
        ...
        <fieldset class="p-4 border rounded-lg bg-gray-50 space-y-4 @if(!$otpVerified) opacity-50 pointer-events-none @endif">
                    ...
                    <div class="absolute inset-y-0 right-0 pr-3 flex items-center text-sm leading-5">
                        <button type="button" @click="show = !show" class="text-gray-500 hover:text-gray-700">
                            ...
                            <svg x-show="show" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" style="display: none;">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.542-7 1.274-4.057 5.064-7 9.542-7 1.845 0 3.543.586 4.968 1.584M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.58 17.58L6.42 6.42" />
                            </svg>
                        </button>
                    </div>
                </div>
            ...
            <div x-data="{ show: false }">
                ...
                <div class="relative mt-1">
                    {{-- MODIFIKASI: Menambahkan x-ref dan event keydown.enter untuk submit form --}}
        ...
        <div class="flex items-center justify-between pt-4">
            {{-- Tautan ini akan berada di sebelah kiri --}}
            <a class="text-sm font-medium text-emerald-600 hover:text-emerald-700 transition ease-in-out duration-150" href="{{ route('login') }}" wire:navigate>
                {{ __('Sudah punya akun?') }}
            </a>

            {{-- Tombol ini akan berada di sebelah kanan --}}
            <button type="submit"
                ...
                {{-- Menggunakan variabel yang konsisten dari form sebelumnya --}}
                {{ !$otpVerified ? 'disabled' : '' }}>

                {{-- Pastikan wire:target sesuai dengan nama method di Livewire component --}}
