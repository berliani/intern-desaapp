    ...
    public function login()
    {
        try {
            $this->validate([
                ...
                'loginField.required' => 'Email atau Telepon wajib diisi.',
        ...
        // 2. Cek Rate Limiting (untuk keamanan)
        $this->ensureIsNotRateLimited();

        // 3. Siapkan Kredensial untuk Login
        $isEmail = filter_var($this->loginField, FILTER_VALIDATE_EMAIL);
        $credentials = [
            $isEmail ? 'email' : 'telepon' => $this->loginField,
        ...
        if (!Auth::attempt($credentials, $this->remember)) {
            ...
            $this->generateCaptcha(); // Buat captcha baru jika login gagal
            return;
        }
...
<div>
    ...
    <div class="mb-8 relative">
        ...
        <div class="relative">
            ...
            <h2 class="text-3xl font-bold bg-gradient-to-r from-emerald-700 to-emerald-500 bg-clip-text text-transparent">Selamat Datang!</h2>
            <p class="text-gray-600 mt-2 flex items-center">
                <svg class="w-4 h-4 mr-1 text-emerald-500" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11a1 1 0 10-2 0v3.586L7.707 9.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L11 10.586V7z" clip-rule="evenodd"></path></svg>
    ...
    <form wire:submit="login" class="space-y-5">
        ...
        <div>
            <x-input-label for="loginField" :value="__('Email atau Nomor Telepon')" class="text-gray-700 font-medium mb-1" />
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <svg class="h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207" /></svg>
                </div>
                <x-text-input
        ...
        <div class="mt-4">
                ...
                <div class="flex items-center justify-around w-48 h-16 px-2 bg-gray-200 border rounded-md overflow-hidden" style="background-image: url('data:image/svg+xml,%3Csvg width=\'60\' height=\'60\' viewBox=\'0 0 60 60\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cg fill=\'none\' fill-rule=\'evenodd\'%3E%3Cg fill=\'%23d4d4d8\' fill-opacity=\'0.4\'%3E%3Cpath d=\'M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z\'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E');">
                    @if(!empty($generatedCaptcha))
                        @foreach (str_split($generatedCaptcha) as $char)
                            @php
                                $hue = rand(0, 360); $saturation = rand(70, 95); $lightness = rand(25, 45);
                                $color = "hsl({$hue}, {$saturation}%, {$lightness}%)";
                                $rotation = rand(-25, 25); $font_size = rand(22, 32);
                                $top_offset = rand(-5, 5); $font_weight = rand(400, 800);
                            @endphp
            ...
            <x-text-input wire:model="captcha" id="captcha" type="text" class="block mt-2 w-full" placeholder="Masukkan captcha di atas" required />
